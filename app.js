 ...
 *
 * Note on Reader: the Bible reader is one route (#bible) but internally
 * manages two screens — a selection screen and a reading screen — see
 * js/reader.js. Reader.openLocation() + App.goTo('bible') always jumps
 * straight to the reading screen, whether or not the hash actually changes.
 */
...
const App = (() => {
  ...
  function goTo(route) {
    // If we're already on that hash, hashchange won't fire on its own —
    // force a re-render so e.g. "Open" from Bookmarks while already on
    // the Bible tab still jumps to the reading screen.
  ...
  return { init, applyTheme, applyReaderPrefs, goTo };
})();
