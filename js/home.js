  ...
  function wireQuickAccess(container) {
    container.querySelectorAll('[data-quick]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (action === 'ot') { Reader.openLocation(Storage.getSettings().version, 'GEN', 1); App.goTo('reader'); }
        else if (action === 'nt') { Reader.openLocation(Storage.getSettings().version, 'MAT', 1); App.goTo('reader'); }
